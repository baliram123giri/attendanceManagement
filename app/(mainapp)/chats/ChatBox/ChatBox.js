...
const ChatBox = () => {
    const { currentChat, sendMessage, messages } = useContext(ChatContex)
                ...
                <div className='p-5 h-[90%] overflow-auto'>
                    {messages && messages?.map((message, index) => {
                        return <div key={index} ref={scrollRef} >
                            <ChatMessages time={message?.createdAt} receiver={user?._id !== message?.senderId} message={message?.text} />
